#---------load function---------
sumresult <- function(qvalue, causal.otus, noncausal.otus, fdr.target=0.1) {
...
type1error <- function(qvalue,fdr.target=0.1) {
...
ADMIC.Basic <- function(data, Y, C = NULL,pseudo.count = 1, 
  ...
  high.taxa <- c()
  cut <- 0.8
  while(length(high.taxa) == 0){
    high.abun.taxa <- which(abun > quantile(abun, cut))
    high.pre.taxa <- which(prop.presence > cut*max(prop.presence))
    high.taxa <- intersect(high.abun.taxa, high.pre.taxa)
    cut <- cut - 0.1
  }
  n.tax <- ncol(data)
  n.sam <- nrow(data)
...
#------background data--------------
theta <- read.csv("data/theta_Healthy.csv", row.names = 1) %>% as.numeric()
pi <- read.csv("data/pi_Healthy.csv", row.names = 1)
pi <- t(pi)

DM_sim <- function(data){
  ###############
  #    DM
  ###############
  th <- quantile(pi)
  n.method <- 9
  n.c <- 5
  SEN <- matrix(0,nrow=n.method,ncol=n.c)
  FDR <- matrix(0,nrow=n.method,ncol=n.c)
  AUPR <- matrix(0,nrow=n.method,ncol=n.c)
  controlFDR <- matrix(0,nrow=4*n.method,ncol=n.c)
  ...
  for(a in 1:n.c){
    ...
    #----DA taxa-----
    dt0 <- rep(0, n.otus)
    candi.otu <- c(which(pi >= th[3]))
    otu.Y.only <- sort(sample(candi.otu, round(0.1*n.otus)))
    dt0[otu.Y.only] <- 1
    causal.otus <- c(otu.Y.only)
    
    noncausal.otus <- setdiff(1:n.otus, causal.otus)
    beta.otu1 <- rep(beta, length(otu.Y.only)/2)
    beta.otu2 <- rep(-beta, (length(otu.Y.only)-length(beta.otu1)))
    beta.otu <- c(beta.otu1, beta.otu2)
    beta.otu <- sample(beta.otu, length(beta.otu))
    ...
    disp <- (1-theta)/theta
    otu.table.sim  <-  rdirichlet(n = n.sam, disp*pi)
    
    # Introducing the effects of Y
    otu.table.sim[, otu.Y.only] <- otu.table.sim[, otu.Y.only] * exp(Y %*% t(beta.otu))
    ...
    #  Filtering otus
    # -----------------------------------
    prop.presence0 <- colMeans(otu.table.sim[control,] > 0)
    prop.presence1 <- colMeans(otu.table.sim[case,] > 0)
    otus.keep0 <- which(prop.presence0 >=0.05)
    otus.keep1 <- which(prop.presence1 >=0.05)
    otus.keep <- intersect(otus.keep0, otus.keep1)
    otu.table.filter <- otu.table.sim[, otus.keep]
    ...
    n.sam <- nrow(data)
    n.sam.control <- length(which(Y==0))
    n.sam.case <- length(which(Y==1))
    n.tax <- ncol(data)
    #Adding pseudo count
    data[which(data == 0)] <- 1
    ...
    # ADMIC-Basic
    #############
    res.ADMIC.basic <- ADMIC.Basic(otu.table.filter,Y)
    q0 <- res.ADMIC.basic$q.basic
    fq0 <- 1-q0
    aupr.cor0 <- performance(prediction(fq0,dt),"aucpr")
    AUPR[9,a] <- unlist(slot(aupr.cor0,"y.values"))
    ...
    AUPR[1,a] <- unlist(slot(pred.clr,"y.values"))
    ...
    AUPR[2,a] <- unlist(slot(aupr.maaslin2,"y.values"))
    ...
    p.aldex2 <- res.aldex2$wi.ep
    q.aldex2 <- res.aldex2$wi.eBH
    fq.aldex2 <- 1-q.aldex2
    aupr.aldex2 <- performance(prediction(fq.aldex2,dt),"aucpr")
    AUPR[3,a] <- unlist(slot(aupr.aldex2,"y.values"))
    ...
    AUPR[4,a] <- unlist(slot(pred.linda,"y.values"))
    ...
    AUPR[5,a] <- unlist(slot(aupr.ancombc,"y.values"))
    ...
    AUPR[7,a] <- unlist(slot(pred.zicoseq,"y.values"))
    ...
    AUPR[8,a] <- unlist(slot(pred.adapt,"y.values"))
    
    otu.clr <- sumresult(q.clr,causal.otus.filter,noncausal.otus.filter)
    SEN[1,a] <- otu.clr$sen
    FDR[1,a] <- otu.clr$fdr
    if(a == 1){
      FDR[1,a] <- type1error(q.clr)
    }
    
    otu.maaslin2 <- sumresult(q.maaslin2,c.otu,n.otu)
    SEN[2,a] <- otu.maaslin2$sen
    FDR[2,a] <- otu.maaslin2$fdr
    if(a == 1){
      FDR[2,a] <- type1error(q.maaslin2)
    }
    
    otu.aldex2 <- sumresult(q.aldex2,causal.otus.filter,noncausal.otus.filter)
    SEN[3,a] <- otu.aldex2$sen
    FDR[3,a] <- otu.aldex2$fdr
    if(a == 1){
      FDR[3,a] <- type1error(q.aldex2)
    }
    
    otu.linda <- sumresult(q.linda,causal.otus.filter,noncausal.otus.filter)
    SEN[4,a] <- otu.linda$sen
    FDR[4,a] <- otu.linda$fdr
    if(a == 1){
      FDR[4,a] <- type1error(q.linda)
    }
    
    otu.ancombc <- sumresult(q.ancombc,causal.otus.filter,noncausal.otus.filter)
    SEN[5,a] <- otu.ancombc$sen
    FDR[5,a] <- otu.ancombc$fdr
    if(a == 1){
      FDR[5,a] <- type1error(q.ancombc)
    }
    
    otu.locom <- sumresult(q.locom,causal.otus.filter,noncausal.otus.filter)
    SEN[6,a] <- otu.locom$sen
    FDR[6,a] <- otu.locom$fdr
    if(a == 1){
      FDR[6,a] <- type1error(q.locom)
    }
    
    otu.zicoseq <- sumresult(q.zicoseq,causal.otus.filter,noncausal.otus.filter)
    SEN[7,a] <- otu.zicoseq$sen
    FDR[7,a] <- otu.zicoseq$fdr
    if(a == 1){
      FDR[7,a] <- type1error(q.zicoseq)
    }
    
    otu.adapt <- sumresult(q.adapt,causal.otus.filter,noncausal.otus.filter)
    SEN[8,a] <- otu.adapt$sen
    FDR[8,a] <- otu.adapt$fdr
    if(a == 1){
      FDR[8,a] <- type1error(q.adapt)
    }
    ...
    FDR[9,a] <- otu.lm$fdr
    if(a == 1){
      FDR[9,a] <- type1error(q0)
    }
    
    #---------------------------------------
  }
  myres <- list(SEN,FDR,AUPR)
  return(myres)
}
...
start <- Sys.time()
myresults <- foreach(i = 1:100, .errorhandling = "pass") %dopar% {
  DM_sim(i)
}
end <- Sys.time()
...
FDR <- fdr/n
AUPR <- myaupr/n
methodname <- c("wilcox-clr","MaAsLin2", "Aldex2","Linda","ANCOMBC","LOCOM","ZicoSeq","ADAPT","ADMIC-Basic")
